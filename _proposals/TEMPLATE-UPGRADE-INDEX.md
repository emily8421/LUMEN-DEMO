...
| `directory-partition-principles.md` | `global-rules §5` 补「代码目录划分依据」五条通用原理（部署边界 / 架构分层 / 特性纵切 / 契约单源 / 生命周期）+ web-fullstack-profile §4 依据→推荐树映射；与 #357 正交（#357 管根目录，本份管代码目录内部分层维度） | 低 | **已提交 [#370](https://github.com/emily8421/ai-project-template/issues/370)（2026-08-18，OPEN）——勿重复提交** |
