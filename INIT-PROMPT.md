...
> 本节是 vision-start 项目的主入口；§1（从已填好的 00-02 生成 03-08）适用于 00-02 已人工写好的项目。
...
- 两层分离：需求层(00-03)完整描述系统、跨阶段稳定、不按阶段裁剪；
  执行层(project-rules §1 + 04-08 + design-* + verification)描述"现在建什么"。
- 积累式(project-rules §6)：设计文档「完整骨架 + 阶段标签 + 状态」，只增不删、不建分阶段文档。
...
4) 详细设计（完整骨架 + 当前阶段细节）
   ...
   - verification-plan：测试策略 + REQ→用例追溯矩阵（当前阶段）

【硬约束】
1. 完整不裁剪：01/02 装下整个愿景，每条可回锚点为证；不得因阶段而删功能
2. 阶段是提议非决定：03 §3 路线图标"待确认"，阶段标签据此打
3. 框架一次完整：04-08 + design-* 铺全部要素（含 P2/愿景骨架），不只写当前阶段
4. 只增不删、原位增量（project-rules §6）
