...
| API-054 | GET | /api/admin/users/{user_id}/spaces | 查询用户所属空间 + 各空间角色（admin 域只读） | [P2] | 维护态批5·待实现 | 仅全局 admin（4030）；用户不存在 4004 | REQ-050 |
...
| API-054 `GET /api/admin/users/{user_id}/spaces` | — | `{code,msg,data:[{space_id,space_code,space_name,role,joined_at}]}` | 全局 admin（member 4030）；用户不存在 4004 | `lumen_space_members`、`lumen_spaces` | **维护态批5·待实现**（REQ-050，Sprint-30）；复用 `list_user_spaces` + `list_spaces` + `list_memberships`，service/repo 零改动；前端用户详情抽屉读此接口 |
