...
| 当前状态 | Phase1 全量验收已记录，结论为 Conditional Go（Demo closure）；Phase1.5A 已完成 TC-P1-015/016；Phase2A 已完成 TC-P2-LINK-001 / TC-P2-TAG-001 / TC-P2-QUICK-001，个人知识组织三个 vertical slice 验收通过。Phase1.5B 仍待 RG；**Phase2B 范围已确认（2026-07-30）+ 设计就绪 + 数据外发风险已接受；REQ-014 已完成 v1.1.0（TC-P2-AI-001 通过）；Sprint-21 Doc-First UX slice 3a/3c/3d 已完成本地 build + 用户 smoke + TC-P1-014/015 回归（2026-08-01 用户确认）**；愿景项待升阶段。 |
| 最后更新 | 2026-08-01（Sprint-21 slice 3b 单列阅读/编辑切换 + 顶栏 + inspector tabs 完成，含 smoke 反馈修复 download 中文标题 / quick-entry A / 标签内联新建；slice 3d 导入弹窗化 + TC-P1-015/014 回归；保留 2026-07-20 Phase2A closure 验收记录） |
...
| TC-P1-015 | REQ-037 批量导入 | 拖入文件夹 / 多文件后全部 `.md`/`.txt` 入库可搜可问答；标题保留路径前缀；同名跳过 | [P1] | 通过（Sprint-16 自动化 + Chrome headless drop-zone smoke）；**Sprint-21 slice 3d 导入弹窗入口形态回归通过**（2026-08-01 用户 smoke 确认） |
