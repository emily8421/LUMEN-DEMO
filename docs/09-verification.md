...
| 当前状态 | Phase1 全量验收已记录，结论为 Conditional Go（Demo closure）；Phase1.5A 已完成 TC-P1-015/016；Phase2A 已完成 TC-P2-LINK-001 / TC-P2-TAG-001 / TC-P2-QUICK-001，个人知识组织三个 vertical slice 验收通过。Phase1.5B 仍待 RG；**Phase2B 范围已确认（2026-07-30）+ 设计就绪 + 数据外发风险已接受；REQ-014 已完成 v1.1.0（TC-P2-AI-001 通过）；REQ-013a/024 主题时间线 TC-P2-TL-001 已通过本地自动化验证 + frontend build + 运行态 API smoke + Edge headless 浏览器 smoke；Sprint-21 Doc-First UX 已完成**；**REQ-039 文档目录树 TC-P2-FOLDER-001 已完成后端核心 + API-029 `preserve_structure` 导入保留结构 + 前端文件管理器基础能力（含 API-038 单文档移动）验证；浏览器人工 smoke 已通过（2026-08-03 用户确认），自动化 smoke 后续补**；愿景项待升阶段。 |
| 最后更新 | 2026-08-03（Sprint-20：task-030 API-033 + 前端时间线视图实现；`test_timeline` 7/7 OK，timeline/document/tag/doc_links 回归 38/38 OK，backend discover 190 OK skipped=2，frontend build 通过，运行态 API smoke + Edge headless 浏览器 smoke 通过。Sprint-22：task-027/028/029 已完成） |
...
| TC-P1-015 | REQ-037 批量导入 | 拖入文件夹 / 多文件后全部 `.md`/`.txt` 入库可搜可问答；标题保留路径前缀；同名跳过（**Phase2B folder-tree 扩展**：`preserve_structure=true` 保留真实目录结构，见 TC-P2-FOLDER-001） | [P1] | 通过（Sprint-16 自动化 + Chrome headless drop-zone smoke）；**Sprint-21 slice 3d 导入弹窗入口形态回归通过**（2026-08-01 用户 smoke 确认）；**Phase2B `preserve_structure` 后端/API 扩展通过**（task-028，2026-08-03）；前端文件管理器基础能力已实现并通过用户浏览器 smoke（2026-08-03），自动化 smoke 后续补 |
...
| TC-P2-TL-001 | REQ-013a；REQ-024；API-033；migration 012 | Sprint-19 已落地；folder-tree 基础能力已完成；候选 A 不建 timeline 事件表；API-033 已实现 | 调用主题时间线：`q` 标题 / chunk 命中、`tag_ids` 标签入口、不传 `q` 空间总览、4 类事件、actor、密度热条、零命中 / 非法参数 / 大集合降级；前端进入时间线视图并加载结果 | 仅当前用户可见文档事件返回；越权事件零泄露；`created/updated/tagged/linked` 事件与 actor 规则正确（linked=`null`）；density 返回 `event_count/level/ratio/window`；大集合 `degraded=true` 且列表有上限 | `tests/backend/test_timeline.py`（7/7 OK）+ `tests/backend.test_timeline tests.backend.test_document tests.backend.test_tags tests.backend.test_doc_links` 回归（38/38 OK）+ backend discover 190 OK（skipped=2，embedding torch DLL 警告按 text-only fallback 继续）+ `npm run build`（frontend，259 modules）+ 运行态 API smoke 通过（OpenAPI 含 API-033，创建文档/标签，关键词 timeline、tag timeline、空 q 422）+ Edge headless CDP 浏览器 smoke 通过（登录、切到时间线、搜索 Phoenix、事件列表命中，density=1） | **通过**（task-030，2026-08-03；自动化 + 运行态 API + Edge headless 浏览器 smoke） |
| TC-P2-FOLDER-001 | REQ-039；`lumen_folders` / `lumen_documents.folder_id`；API-034..038；API-029 `preserve_structure` | folder-tree 设计 FT-C-* 确认 + 06/07 契约回填；migration 011 已落地；API-034..038 已实现 | 新建 / 移动 / 排序 / 删除文件夹（API-034..037）；单文档移动到 folder / 根目录（API-038）；导入文件夹后 `preserve_structure=true` 保留目录结构、`=false` 退回标题前缀；防环 / 跨空间 / 重名 / 删非空 folder 分别验证；文档可见性不因 folder 归属泄露 | folder CRUD / 移动 / 排序正确；单文档移动只更新 `folder_id`、不新增版本；`UNIQUE(space_id,parent_id,name)` 重名→4090；防环 / 跨空间→4220；删非空→4090；folder **不独立设权限**，文档可见性仍按 `permission` + `space_id` 过滤；现有文档 `folder_id=null` 向后兼容；`preserve_structure=true` 按路径段建/复用 folder，`false` 保持旧平铺标题 | `tests/backend/test_folder.py`（19/19 OK，task-027）+ `tests/backend/test_document.py`（API-038 service/API 覆盖，task-029）+ `tests/backend/test_imports.py` / `test_import_api.py`（11/11 OK，task-028）+ 临时 PG folder/import smoke（2026-08-03）+ `volta run --node 22.17.1 npm run build`（task-029）+ 运行态 API smoke（API-038 出现在 OpenAPI；临时文档移动到 folder 后 `folder_id` 读回一致并已清理）+ 用户浏览器 smoke（2026-08-03） | **后端/API + 前端 build + 运行态 API smoke + 用户浏览器 smoke 已通过**（task-027 + task-028 + task-029，2026-08-03）；浏览器自动化 smoke 后续补 |
- REQ-027 PDF 导出已提前到 Phase1.5B，验证口径见 TC-P1-017。
- **REQ-013a 主题时间线 / REQ-024 密度热条**：TC-P2-TL-001 已列入上表；task-030 已通过自动化验证、运行态 API smoke 与 Edge headless 浏览器 smoke；详见 `docs/design/timeline.md`。
- **REQ-039 文档目录树（Phase2B 第三 slice）**：TC-P2-FOLDER-001——文件夹树 CRUD / 移动 / 排序 + 单文档移动 + 导入保留目录结构（`preserve_structure`）后端/API、前端 build、运行态 API smoke 与用户浏览器 smoke 已通过；folder 不独立设权限，文档可见性仍按 permission；防环 / 跨空间 / 重名 / 删非空 folder 拒绝；浏览器自动化 smoke 后续补；详见 `docs/design/folder-tree.md`。
...
| 2026-08-03 | Sprint-22 前端文件管理器与单文档移动补齐（task-029；REQ-039/004） | 通过（后端针对性测试 + 前端 build + 运行态 API smoke + 用户浏览器 smoke；浏览器自动化 smoke 后续补） | API-038 `PATCH /api/documents/{document_id}/folder`：文档可见且可写，目标 folder 必须当前空间，`folder_id=null` 移到根；移动只更新 `lumen_documents.folder_id`，不新增版本 / 不重建索引。前端：左侧 `FolderTree` 文档行右键菜单“移动到”；folder 菜单受控关闭；文件夹 inline 新建 / 重命名；移动后刷新文档列表与已加载 folder 计数。验证：`.venv\Scripts\python.exe -m unittest tests.backend.test_document tests.backend.test_folder tests.backend.test_imports tests.backend.test_import_api` **38/38 OK**；`volta run --node 22.17.1 npm run build` 通过（255 modules）；运行态 OpenAPI 已出现 API-038，临时文档 API smoke 移动成功且 `folder_id` 读回一致，临时数据已清理；用户浏览器测试确认无问题（2026-08-03）。残留：Playwright / Browser 自动化通道本轮不可用，自动化点击 smoke 后续补；目标列表首版仅含已加载 folder。 |
| 2026-08-03 | Sprint-20 主题时间线（task-030；REQ-013a/024） | 通过（后端自动化 + 前端 build + 运行态 API smoke + Edge headless 浏览器 smoke） | Candidate A 时间线不建事件表，API-033 `GET /api/spaces/{id}/timeline` 实时聚合 documents / tag_links / doc_links / chunks；migration 012 仅加时间索引；前端接入独立时间线视图（关键词、标签入口、密度热条、事件列表）。本轮 smoke 发现并修复 demo runtime 新建文档 `created_at/updated_at` 为空导致 timeline 无事件的缺口，补 `DemoRepository.create_document/update_document` 时间戳与回归用例。验证：`tests.backend.test_timeline` **7/7 OK**；timeline/document/tag/doc_links 回归 **38/38 OK**；backend discover **190 OK(skipped=2)**（embedding torch DLL 权限警告按 text-only fallback 继续）；frontend `npm run build` 通过（259 modules）；运行态 API smoke 通过（OpenAPI 含 API-033，创建文档/标签，关键词 timeline、tag timeline、空 q 422）；Edge headless CDP 浏览器 smoke 通过（登录、切到时间线、搜索 `Phoenix`，事件列表命中 `Phoenix Sprint20 Timeline Smoke`，density=1，无错误文本）。残留：真实 PG 大数据性能未实测；`linked` actor 按 TL-C-011 返回 `null`；无新增依赖 / 无 timeline 事件表。 |
...
- **Phase2B 范围、进入 / 退出标准、数据外发风险接受方式（RG-008）与验证包均已确认 / 就绪（2026-07-30）**；REQ-014 AI 润色已完成并通过 TC-P2-AI-001；REQ-013a/024 主题时间线已完成并通过自动化验证、运行态 API smoke 与 Edge headless 浏览器 smoke。
