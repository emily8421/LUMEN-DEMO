...
| 覆盖 REQ | Phase1：REQ-001..REQ-011、REQ-036；Phase1.5A：REQ-037/038；Phase1.5B：REQ-027；Phase2A：REQ-026/012/025；**Phase2B：REQ-014 首批核心（后端已实现，RG-008 升 Go，2026-07-30）+ REQ-013a 主题时间线 / REQ-024 第二 slice（已设计，编码移 folder-tree 后）+ REQ-039 文档目录树第三 slice**；愿景验证项待升阶段 |
| 当前状态 | Phase1 全量验收已记录，结论为 Conditional Go（Demo closure）；Phase1.5A 已完成 TC-P1-015/016；Phase2A 已完成 TC-P2-LINK-001 / TC-P2-TAG-001 / TC-P2-QUICK-001，个人知识组织三个 vertical slice 验收通过。Phase1.5B 仍待 RG；**Phase2B 范围已确认（2026-07-30）+ 设计就绪 + 数据外发风险已接受；REQ-014 已完成 v1.1.0（TC-P2-AI-001 通过）；Sprint-21 Doc-First UX 已完成**；**REQ-039 文档目录树 TC-P2-FOLDER-001 已完成后端核心 + API-029 `preserve_structure` 导入保留结构 + 前端文件管理器基础能力（含 API-038 单文档移动）验证；浏览器人工 smoke 已通过（2026-08-03 用户确认），自动化 smoke 后续补**；愿景项待升阶段。 |
| 最后更新 | 2026-08-03（Sprint-22：folder-tree 后端核心 + PG CTE smoke；API-029 `preserve_structure` 后端/API + 临时 PG import smoke + frontend build；task-029 API-038 单文档移动 + 前端文件管理器基础能力通过后端 38 tests + frontend build + 运行态 API smoke + 用户浏览器 smoke；timeline 仍在 folder-tree 后） |
...
- **REQ-013a 主题时间线 / REQ-024 密度热条（Phase2B 首批·第二 slice·已设计，编码移 folder-tree 后）**：TC-P2-TL-001（待 folder-tree 后实现）——**主题时间线（关键词/标签驱动）**：`q` 命中仅可见集内（标题 ILIKE + chunk.ts_vector），`tag_ids` 标签主题入口，不传 `q`=空间总览；候选 A 实时聚合（UNION ALL 4 类事件：created/updated/tagged/linked）+ **actor**（created/updated=owner，tagged=created_by，**linked=null**）；时间轴仅含当前用户可见文档事件；密度热条 4 档色阶（0/1/2/3）**+ 量化 ratio**（事件数 + 渲染去重）；零命中空态；大集合聚合 / 采样降级（>500 文档或 >2000 事件，`degraded=true`）+ 列表逃生舱；**越权事件不泄露（红线）**；TL-C-001..011 已确认，详见 `docs/design/timeline.md`。
