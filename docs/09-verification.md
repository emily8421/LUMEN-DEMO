...
| 最后更新 | 2026-08-01（Sprint-21 slice 3d 导入弹窗化完成本地 build + 用户 smoke + TC-P1-015/014 回归；保留 2026-07-20 Phase2A closure 验收记录） |
...
| TC-P1-014 | REQ-011 P1B 工作台重设计 | TopBar + Nav Rail + Context Pane + Workspace 三层布局；Context Pane 随视图变化；900px 桌面宽度不横向破版；信息密度达标；P0/P1A 能力不回退 | [P1] | 通过（构建 + Chrome / Edge 900px smoke）；**Sprint-21 slice 3a/3c/3d 回归通过**（栏默认收起 + 首页默认落地 + documents 空态/返回 + 导入入口迁移后栏显隐 / 900px 不回退，2026-08-01 用户本地 smoke 确认） |
