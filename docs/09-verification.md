...
| TC-P2-TAG-001 | REQ-012；`lumen_tags` / `lumen_tag_links`；API-014 / API-027 / API-031 | Phase2A 最小版已细化（扁平标签 + 独立标签视图 + 单标签筛选 + 文档详情打标签）；首个 slice REQ-026 已完成 | 创建 / 更新 / 归档标签（API-014/027）；给可见文档打标签 / 移除（API-031）；按标签筛选文档（`GET /api/documents?tag_id=`）；跨空间 / 无权限文档不进入 document_count 与筛选结果 | 标签只在当前空间可见；同空间 normalized_name 重名返回 4090；document_count 只统计当前用户可见文档；归档标签不破坏既有 tag_links 历史关联；打标签需文档可写 + 标签同空间 | `tests/backend/test_tags.py`（标签 CRUD + 权限 + document_count + 打标签 / 移除）+ 前端 UI smoke 待 Task B | Phase2A-已设计·待实现 |
