...
| 当前状态 | P1 验证计划已确认；Sprint-2~6 降级口径验收已执行（见 §2 状态、§5 验收记录、§6 未验证项） |
| 最后更新 | 2026-07-09 |
...
| REQ | 验收用例 | 阶段 | 状态 |
|---|---|---|---|
| REQ-001 隔离 | brightlite 账号查询 nova-internal 文档 = 0 命中 | [P1] | 条件通过（降级口径·内存） |
| REQ-002 切换 | 切换后搜索 / 问答只反映目标空间 | [P1] | 条件通过（降级口径·内存） |
| REQ-003 权限 | 作者私有文档，同空间他人搜不到、问答不引用 | [P1] | 条件通过（降级口径·内存） |
| REQ-004 CRUD | 创建 / 读 / 改 / 删均成功，删后不可检索 | [P1] | 条件通过（降级口径·内存） |
| REQ-005 行内编辑 | 编辑保存后内容持久、再打开一致 | [P1] | 条件通过（降级口径·内存） |
| REQ-006 版本 | 3 次修改 → 3 版本 → 能恢复指定版本 | [P1] | 条件通过（降级口径·内存） |
| REQ-007 搜索 | 已知关键词返回正确文档 | [P1] | 条件通过（降级口径·内存关键词匹配） |
| REQ-008 RAG | 库内问答正确 + 标来源；库外回复"未找到" | [P1] | 条件通过（降级口径·不调 LLM） |
| REQ-009 导入 | 导入 .pdf 后可搜、可问答引用 | [P1] | 条件通过（仅 `.md`/`.txt`；真实 PDF 未验证） |
| REQ-010 OCR | 中文白板照片 OCR 后可搜 | [P1] | 后续阶段（OCR 未实现，降级移出 P1 必过） |
| REQ-011 桌面端 | Chrome / Edge 完成上述全部 | [P1] | 条件通过（降级口径·Edge Headless + Chrome 人工 smoke） |
| REQ-036 术语管理 | 新建空间术语后，文档识别该词，问答优先使用空间定义且不被同名全局术语覆盖 | [P1] | 条件通过（降级口径·内存） |
