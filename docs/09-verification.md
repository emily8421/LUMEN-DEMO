...
| 覆盖 REQ | Phase1：REQ-001..REQ-011、REQ-036；Phase1.5A：REQ-037/038；Phase1.5B：REQ-027；Phase2A：REQ-026/012/025；**Phase2B：REQ-014 首批核心（后端已实现，RG-008 升 Go，2026-07-30）+ REQ-013a 主题时间线 / REQ-024 第二 slice（已设计，编码移 folder-tree 后）+ REQ-039 文档目录树第三 slice 候选（待 Sprint-22）**；愿景验证项待升阶段 |
...
| 最后更新 | 2026-08-02（folder-tree 设计回填：新增 TC-P2-FOLDER-001 + 扩展 TC-P1-015 `preserve_structure`；**timeline 重定位：TC-P2-TL-001 改为主题时间线（关键词/标签驱动 + actor + 密度 ratio + 零命中），Phase2B 第二 slice REQ-013a/024，编码移 folder-tree 后，TL-C-001..011**；Sprint-21 验收记录见前版） |
...
| TC-P1-015 | REQ-037 批量导入 | 拖入文件夹 / 多文件后全部 `.md`/`.txt` 入库可搜可问答；标题保留路径前缀；同名跳过（**Phase2B folder-tree 扩展**：`preserve_structure=true` 保留真实目录结构，见 TC-P2-FOLDER-001） | [P1] | 通过（Sprint-16 自动化 + Chrome headless drop-zone smoke）；**Sprint-21 slice 3d 导入弹窗入口形态回归通过**（2026-08-01 用户 smoke 确认）；Phase2B `preserve_structure` 扩展待 TC-P2-FOLDER-001 |
...
| TC-P1-015 | REQ-037 | 多个 `.md` / `.txt` 文件或文件夹；Sprint-21 slice 3d 入口形态改为 Documents toolbar「导入」→ 居中 modal | 拖入文件夹 / 多文件后批量导入；3d 追加检查 modal 打开 / 关闭 / 文件选择 / 权限 / 逐条结果 / 导入成功刷新 | 全部入库可搜可问答；标题保留路径前缀；同名跳过；入口迁移不改变 API-029 / 导入契约；**Phase2B folder-tree 扩展**（`preserve_structure=true` 保留真实目录结构）归 TC-P2-FOLDER-001，本 TC P1 范围不覆盖 | `tests/backend/test_imports.py`（批量成功、失败隔离、同名跳过、可搜可问答）；`tests/backend/test_import_api.py`（API-029 响应契约）；`npm.cmd run build`；Chrome headless drop-zone smoke（登录 → drop 两个带 `webkitRelativePath` 文件 → 导入成功 2 → 搜索 / 问答命中）；Sprint-21 3d：`volta run --node 22.17.1 npm run build` 通过 + 用户 smoke 确认 modal 入口形态回归通过（2026-08-01） | 通过（含 3d 入口形态回归） |
...
| TC-P2-FOLDER-001 | REQ-039；`lumen_folders` / `lumen_documents.folder_id`；API-034..037；API-029 `preserve_structure` | folder-tree 设计 FT-C-* 确认 + 06/07 契约回填；migration 011 已落地（待编码） | 新建 / 移动 / 排序 / 删除文件夹（API-034..037）；导入文件夹后 `preserve_structure=true` 保留目录结构、`=false` 退回标题前缀；防环 / 跨空间 / 重名 / 删非空 folder 分别验证；文档可见性不因 folder 归属泄露 | folder CRUD / 移动 / 排序正确；`UNIQUE(space_id,parent_id,name)` 重名→4090；防环 / 跨空间→4220；删非空→4090；folder **不独立设权限**，文档可见性仍按 `permission` + `space_id` 过滤；现有文档 `folder_id=null` 向后兼容 | `tests/backend/test_folder.py`（19/19 OK，task-027）+ `test_imports.py`（preserve_structure，待 slice 2）+ 浏览器 smoke（待 slice 3 前端） | **后端已通过（task-027，2026-08-03；19 service+API + 45 回归 OK）**；导入 preserve_structure + 前端 smoke 待 slice 2/3 |
...
| 2026-08-03 | Sprint-22 folder-tree 后端核心（task-027；REQ-039） | 通过（后端单测 + 回归；前端 + 导入改造待 slice 2/3） | migration 011 `lumen_folders` + `lumen_documents.folder_id`；entity `Folder` + `FolderORM`；`repository` pg/demo 11 方法（pg `WITH RECURSIVE` 防环 / demo 内存递归）；`service/folder.py`（FolderView + 异常 4003/4004/4090/4220 + CRUD/移动防环跨空间/删非空 4090/排序；document_count 按可见性过滤）；API-034..037 `/api/folders`（token current_space_id；PATCH 用 model_fields_set 区分未传/null；2026-08-02 路径裁定）。验证：`tests.backend.test_folder` **19/19 OK**（18 service + 1 API：CRUD/根层+嵌套/重名 4090 含 parent=null service 兜底/空 name 4220/非成员 4003/parent 跨空间 4220/改名重名/移动含移根/防环 4220 后代+自身/跨空间移动/删空/删非空子夹+文档 4090/排序/部分排序拒绝/document_count 可见性/空间隔离/API 端到端）+ 回归 `test_tags+test_document+test_quick_entry+test_doc_links` **45/45 OK** + `import backend.main` 通过。越界（本轮不做）：导入 preserve_structure（Flow-D-012）/ 前端文件管理器 / folder 独立权限 / 文档 order / 软删除。待办：PG `WITH RECURSIVE` 真实库验证（test_pg_repository 可选）+ 切分支 commit/PR（含 bump MINOR：新增 API endpoint）+ slice 2 导入 + slice 3 前端。 |
