...
> **定位**：技术方案是概要设计的「技术选型与风险」补充（技术栈 / 版本 / 资源 / 安全 / 降级 / readiness gate），**非 OO 五阶段的图纸阶段**；其图（技术栈分层 / 部署拓扑）服务于技术决策，类级 / 交互级图纸在 04 与 `docs/design/`。
...
- **【已落地·2026-08-13 维护态批19 / Sprint-44，v3.8.16】** env 集中：新 `backend/config.py`（pydantic-settings `Settings`：`lumen_env`/`database_url`/`demo_token_key` 显式 alias + `get_settings()` 单例 + `validate_runtime_secrets`）；`db.py` / `main.py` / `auth.py` 三处静态 env 收敛到 settings；弱默认 `LUMEN_DEMO_TOKEN_KEY="local-demo-signing-key"` 生产 fail-closed（`LUMEN_ENV=production` 拒绝启动，补 main.py 生产护栏缺口；test / 本地开发 / demo 受控豁免，理由见 implementation-lifecycle §6.2）。**显式豁免**（非静态配置读取）：`llm_adapter.py` `LLM_<NAME>_*` 动态命名配置（key 运行时拼接）、`embedding.py` `HF_HUB_DISABLE_XET` 环境约束 setdefault（huggingface 导入前注入）。新依赖 `pydantic-settings==2.15.0`。实施口径 `tasks/task-051-config-centralization.md`。
- **【已收口·2026-08-13 维护态批18 / Sprint-43，v3.8.15】** `print()` 与 `logging` 混用（CQ-P2-002 日志统一）：`main.py:44,46` 已随 CQ-P1-001 收口为 `logger`；`pg_repository.py:327` `print` 已改 `logger.warning`（后端生产 print 债清零）；降级 `except` 静默吞已收口——`service/document.py:214` 索引回填、`service/rag.py:233,258` LLM 失败 ×2、`api/auth.py:72` `SpaceAccessError` 回退均补 `logger.warning` 记原因；`ruff.toml` select 加 `T20`（flake8-print）禁 print（`scripts/` smoke / CLI stdout 豁免）。实施口径 `tasks/task-050-log-unification.md`。
...
> **本节待对齐项不强制当前维护态回改**：新增代码必须对齐【已落地】项、不得再引入【待对齐】项的同类问题；【待对齐】项登记为技术债，若启动重构 Sprint，优先处理 **§4.2.1 错误契约收口** 与 **§4.2.4 CI 代码门** 两类（契约稳定性与回归保护风险最高）。超长文件（`repository/pg_repository.py` 1621 行、`demo_repository.py` 1251 行等）按 §4.1 阈值另列拆分计划；前端文件膨胀已由 CQ-P1-008 ratchet（`scripts/check-frontend-file-size.mjs` + 分层阈值基线，.css/App→300 / .ts/.tsx→250）机器守护，`App.tsx` 已随维护态批20 拆三 shell（546→360）+ 维护态批21 抽 `useAppState`（359→90）+ 维护态批22 拆 4 个超限 hooks + 维护态批23 拆 4 个超限 CSS（workspace/local-mount/layout/onboarding 全出基线）+ 维护态批24 拆 6 个超限组件（FolderTree/TermCategoryTree/TopBar/ContextPane/ImportFeature/DocumentInspectorFeature 全出基线，含共享 tree/ 抽取）+ 维护态批25 拆 3 个超限组件（WorkspaceMain/DocumentsFeature/LocalMountPane 全出基线；DocumentsFeature textareaRef 三方共享内聚于 DocumentEditorForm；LocalMountTreeView 二次拆右键菜单/内联输入），剩余 2 个超限文件见 `frontend/.file-size-baseline.json`（`app/useAppState.ts` 339 / `app/useDocuments.ts` 286 两个核心编排例外——强拆需大 prop-drill 或循环依赖破解，ROI 低）。
