...
- **【部分落地·2026-08-12 维护态批14 / mypy B1 Slice A】** 后端 mypy 引入（CQ-P1-002 Slice C 类型注解收益兑现前提）：**Slice A**（PR #148 `de88d3a`）装 `mypy==2.3.0`（支持 `python_version=3.14`）+ 根 `mypy.ini`（默认非 strict，查 `backend/` 不含 tests；ASCII 注释避 Windows gbk 编码坑）+ CI `backend-typecheck` job（advisory 起步 `continue-on-error`，复刻 eslint B1 / `backend-lint`）。首跑基线 **190 error / 28 files**：§3 optional-import 重赋值簇 120 主导（`except ImportError: APIRouter=None` 系有意保留的"依赖未装可 import 测试"防御模式，非死代码）+ `current_space_id: int|None → service(int)` None 传播缺口 45（**mypy 核心价值兑现，ruff/tsc 抓不到**）+ reportlab 缺 stub 8 + 真实类型 bug ~17（`re.match` None 守卫 / `Result.rowcount` 误用 / `ContextVar.reset` 等）。**Slice A 不消错、不 bump**（advisory 未落地 required，复刻 eslint B1 Slice A #146）。验证：`mypy backend` 190 基线复现 + ruff passed + pytest 306 零回归 + CI `backend-typecheck` advisory 红（`continue-on-error` 不阻断）+ 其余 6 job 全绿。**Slice B**（另起 PR）按报告 §5 清零（§3 收口 19 文件 + current_space_id 设计决策 + reportlab override + 真实 bug）→ 移除 `continue-on-error` 升 required → bump `v3.8.11`。实施口径 `docs/research/2026-08-12-backend-mypy-b1-assessment.md`。
