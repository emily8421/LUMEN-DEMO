...
| AI | LLM：OpenAI 兼容接口；Embedding：本机 `BAAI/bge-small-zh-v1.5` | Embedding 512 维；`sentence-transformers` 3.0.x；LLM Demo 默认走公司内网 OpenAI 兼容中转，未配置时显式 Mock | **LLM=Mock（RAG 不调 LLM，返回检索结果+模板）；Embedding=未接入（评估报告 No-Go，torch 导入失败）** |
...
| RG-004 | LLM（OpenAI 兼容） | **Mock** | 未配置；当前 RAG 不调 LLM | 配置公司内网中转 / 外部 OpenAI 兼容 API | REQ-008（问答真实化） |
