...
- **【已落地·P0-2 2026-08-11】** CI 代码门：`.github/workflows/project-check.yml` 加 `backend-test`（pytest `-m "not integration" --strict-markers`，CI 运行不强制阻断）、`frontend-build`（`tsc -b && vite build`，CI 运行不强制阻断）、`backend-lint`（ruff `E4/E7/E9/F`，恒 advisory 记 41 旧债基线）三 job；`frontend-lint`（eslint）**留 P1**（B1 暂缓，见 NFR-006 / 本节留痕）。
