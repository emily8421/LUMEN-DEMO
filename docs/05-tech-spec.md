...
- **【部分落地·main.py 已收口 / 维护态批17 / Sprint-42，v3.8.14】** `print()` 与 `logging` 混用：`main.py:44,46` 两处 `print` 已随 CQ-P1-001 fail-fast 收口为 `logger.info` / `logger.warning`（seed 索引 / demo 容忍降级）；**仍待对齐**（CQ-P2-002 日志统一）：`pg_repository.py:324` `print` + 降级 `except` 静默吞无日志（`service/document.py:201` 索引回填、`rag.py:234,259` LLM 失败）→ 禁 `print`，降级路径必须 `logger.warning(...)` 记原因。
