...
| 客户端渲染 / 生命周期异常 | 显示页面级通用恢复界面，并提供“重新加载”操作 | 根 ErrorBoundary 仅覆盖子树渲染 / 生命周期异常；详细异常仅留浏览器控制台，不展示给用户，也不声称覆盖事件处理、异步回调或根节点初始化异常 |
...
> **本节导读（2026-08-10 结构整理）**：本节是 Phase1.5A–Phase2 前端 UI 设计的**历史演进记录**，多套候选稿按时间叠加。Phase2A–D 已全部实现（见 `ai/project-rules.md` §1 / `docs/09-verification.md`）。
> - **§9.0** 已实现交互路径（Phase1.5A/B）——当前有效。
> - **§9.1–§9.3** 2026-07 实现前候选稿（方向池 / 追溯草案 / 少容器清爽稿 v0.2）——**历史候选稿**；视觉与布局决策中已被 **§9.5 Doc-First 基线**覆盖的部分，**以 §9.5 为准**（§9.5.4 等已显式标注调整项）。
> - **§9.4** 编码边界与进入实现条件——当前有效门禁口径。
> - **§9.5** Doc-First 候选基线（2026-07-31）——**当前有效基线**。
>
> ⚠️ 本节定义的 `PG-P2-*` / `CMP-P2-*` / `PATH-P2-*` / `FL-P2-*` / `UXD-*` ID 体系**仍为当前有效契约**，被 §9.5、§10 及外部 design 文档（ai-polish / timeline / accounts-auth 等）引用，**不得删除**；本次整理仅厘清主从，不动 ID。
>
> 读法：新读者优先 §9.0 + §9.5；§9.1–§9.3 作历史溯源与 ID 定义保留。
...
>
> **历史定位（2026-08-10）**：本节为 2026-07 实现前确认稿，Phase2A–D 已实现。视觉/布局决策中已被 §9.5 Doc-First 基线覆盖的部分（如 CMP-P2-DOC-MODE 编辑/预览模式，见 §9.5.4）**以 §9.5 为准**；本节定义的 `PG-P2-*` / `CMP-P2-*` / `PATH-P2-*` ID 体系仍为有效契约，整理仅加历史标注，不动 ID 与既有结构。
...
2. **左目录 / 右栏默认隐藏**：宽屏也默认收起（区别于 P1B 仅窄屏折叠）。**修订（2026-08-14 用户裁决）：左目录改为默认展开**——文档视图开机即见目录树；右栏维持默认收起（选区 / AI 唤出）。无左栏内容的视图（首页 / 标签 / 时间线）恒不显示，与该原则不冲突。偏好 key 升 `lumen-demo-pane-layout-v2`，老偏好作废。
...
| 视图 | 左目录 | 右栏 | 说明 |
|---|---|---|---|
| 文档阅读 / 编辑（PG-P2-003） | **默认展开（2026-08-14 修订，原默认收起）** | 默认收起（选区 / AI 才唤出） | 开机即见目录树；沉浸模式一键收起 |
