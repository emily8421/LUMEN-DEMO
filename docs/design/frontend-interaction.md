...
| PATH-P2-008 | Vault 兼容来源（REQ-018，Phase2C） | 选择本地 vault → 选择“导入数据库”或“仅本地挂载” → 左侧文件管理器显示上下两层 | 导入数据库后出现在上层 LUMEN 知识库并获得完整能力；仅本地挂载出现在下层“本地挂载”，只对本人 / 当前设备可见 | 浏览器授权失败、权限失效、文件删除 / 重命名、未支持格式均给出本地状态；仅本地挂载不进入团队搜索 / RAG | 推荐：RG-009 已 Go（2026-08-05），进入 Phase2C 编码（Sprint-23C 待启动）；交互上明确“本地挂载 != 已入库” |
