...
> 本文同时承担两层职责：一是记录 Phase1（功能范围 `[P1]` · 交付物形态 Demo）已实现的前端交互基线；二是沉淀 Phase2 UI 实现前确认稿候选。
> 当前少容器清爽稿已作为实现前 UI 确认版候选回填到 `docs/08-dev-plan.md` Sprint-11 与 `docs/09-verification.md` TC-P2-UI 草案；但 Phase2 尚未正式启动，本文不直接授权编码。
...
| 输入来源 | 03、04 §1.2 / §5（Flow-001/002）、05、07、08、09；`docs/design/frontend-experience-brief.md`；`docs/research/2026-07-13-ui-prototype-exploration.md`；`docs/research/prototypes/2026-07-14-frontend-ui-reference-absorbed-prototype.html` |
| 覆盖 REQ | P1：REQ-001..011、REQ-036；P2 UI 候选追溯：REQ-012/013/014/025/026（仅作实现前门禁草案，不新增 REQ） |
| 所属 Phase | [P1] 已实现基线 + [P2] UI Gate 草案 |
| 交付物形态 | Demo |
| 当前状态 | P1-已实现；P1A / P1B 前端体验收口已实现并通过构建与 Chrome / Edge 900px smoke；P2 少容器清爽稿暂定为实现前 UI 确认版候选，08/09 已回填 Sprint-11 与 TC-P2-UI 草案；Phase2 与实现任务仍待人工确认 |
| 页面 / 流程 ID | Page-ID（§2.2）/ UF 用户流（§3） |
| UI 原型策略 | P1：代码原型 + smoke；P2：静态 HTML 实现前确认稿 + 后续 Chrome / Edge smoke 草案（见 §8 / §9） |
| 最后更新 | 2026-07-14（重梳文档结构；同步少容器清爽稿、Sprint-11 与 TC-P2-UI 草案） |
| 下游影响 | 08 Sprint-2/4/5/6/9/10；08 Sprint-11（P2-UI-Gate 候选）；09 TC-P1-001~014 与 TC-P2-UI-001~005 草案；Phase2 编码仍需另行确认任务 |
...
| 确认 Phase2 UI 方向 | §8.1、§9.1~§9.3 | 少容器清爽稿暂定按当前稿继续；吸收面包屑、经典 / 探索、层级搜索和图谱降级，但不吸收卡片墙 |
| 判断是否可编码 | §9.4、08 Sprint-11、09 TC-P2-UI | 仍不可直接编码；需人工确认 Phase2 范围、进入 / 退出标准和具体实现任务 |
| 查待确认项 | §11 | P1.5 路由、Phase2 范围、图谱 / 情报墙等仍需人工确认 |
## 1. 职责与边界
...
| Phase2 MVP `[P2]` | UI Gate 草案已回填 | 承接 `docs/design/frontend-experience-brief.md` 与少容器清爽稿：候选方向为分层空间总览、经典 / 探索双模式、标签 / 反链 / 快速录入 / 推荐问题等体验增强；已补 Sprint-11 / TC-P2-UI 草案，但 Phase2 范围和实现任务未确认，不直接进入实现 |
...
### 9.2 Phase2 实现前追溯草案（已回填 08/09 草案）
...
| PG-P2-002 | 经典目录 / 分层导航 | 左侧可调宽目录列，支持多层展开 / 折叠 | 全空间总览、项目 / 主题、子目录、叶子文档 | 展开 / 折叠、选择目录、打开文档、跳转搜索 / 术语 | 文档数量、空目录提示、权限过滤结果 | 推荐：默认展开空间 + 最近 / 当前项目两层；目录宽度可拖拽调整；记忆展开状态先用前端本地状态，不新增后端持久化 |
| PG-P2-003 | 文档工作区 | 可调宽文档列表列 + 可调宽文档主区 + 可调宽右侧状态区 | 当前文件夹文档列表、标题、路径、状态徽章、正文预览 | 预览、编辑、编辑+预览并排、查看版本 / 来源、保存 / 重试 | 来源、权限、版本记录、术语提示 | 推荐：默认预览；阅读与预览合并为一个视图；编辑时提供“编辑 + 预览并排”；目录区、文档列表、编辑预览区和右侧状态区都应可调宽 |
| PG-P2-008 | 面包屑 / 分层搜索吸收项 | TopBar 可点击面包屑 + 左侧当前层级搜索 | 当前空间、项目 / 主题、当前页面路径、当前筛选状态 | 返回空间总览、回到文档层级、过滤当前层级可见内容 | 不跨空间泄露、空结果提示、权限过滤 | 推荐：吸收参考原型的可点击面包屑与层级内搜索；搜索默认只过滤当前空间 / 当前层级可见内容 |
...
| CMP-P2-TREE | 经典目录 | 空间、项目 / 主题、子目录、文档、数量 | 展开 / 折叠、点击跳转、权限过滤后隐藏不可见项 | 空目录、折叠、无权限、长名称 | 推荐：目录列约 280px，支持折叠与拖拽调整宽度；默认展开两层并记忆本地展开状态 |
