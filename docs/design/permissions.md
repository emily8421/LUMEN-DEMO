...
### 0.5 详细类图（DIAG-CLS-PERM-01）

> 图纸驱动编码：空间与权限子系统的类级视图（实体 + `RepositoryProtocol` 契约 + 服务层函数）。类图挂 REQ-001/002/003/043/044；方法签名以 `backend/service/permission.py`、`space.py` 与 `backend/repository/protocol.py` 为准。
