...
| 交付物形态 | Demo |
| 当前状态 | **L0+L1 已实现（2026-08-06，Sprint-25）**；L2-L4 后续 |
...
- L0 + L1（Wave 1 · Sprint-25）：内容源重组 + 首次引导 / 新手清单 / 空状态。**状态：已实现（Sprint-25，2026-08-06）**。
...
- Sprint-25 自动化验证（2026-08-06）：`npm run build` 绿（273 modules）；`scripts/smoke-help-onboarding-browser.mjs` 通过（登录 → 3 步引导 → 首页未建索引提示 → 搜索/问答/时间线空态入口 → 帮助过滤「导入」10→3 → 跳过引导持久化不再弹出）。人工浏览器 smoke 待用户确认后补记。
...
**Sprint-25（L0+L1，2026-08-06）实现回写：**

| 偏差 ID | 代码 / 文档事实 | 原设计（§） | 偏差类型 | 处理结论 |
|---|---|---|---|---|
| H-impl-1 | L1 顶栏帮助弹层先做「分类速查 + 轻量过滤 + 完整手册链接」，未做 Flow-H-002 完整帮助中心 | §1 / Flow-H-002（L2） | 范围裁剪（TC 验收「帮助可检索到导入」以最小过滤满足） | 轻量过滤满足 TC-P2-HELP-001；完整可搜索帮助中心（含高亮 / 完整手册渲染）留 L2 |
| H-impl-2 | 新手清单 / 引导步骤采用「点击跳转即勾选」跟踪，不做真实行为自动检测（如保存文档自动完成步骤） | Flow-H-001 | 实现取舍 | 首版够用；后续可在 useDocuments 保存回调 / 搜索命中时自动勾选 |
| H-impl-3 | 空状态映射：搜索 / 问答做「去新建文档 / 去导入」按钮；标签 / 时间线做「去新建或导入文档」入口；术语视图本身即表单（保持「填写后创建」说明） | §3 空状态映射 | 实现细化 | 与 TC「各视图空状态有下一步」一致 |
| H-impl-4 | 内容源单一来源：`docs/env/user-guide.md` 任务导向重组；弹层速查条目内联但指向完整手册，并标注 Phase2B/2C 章节待补 | §4 | 无偏差 | 单一来源核对通过 |
