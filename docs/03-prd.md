...
| P1.5A 个人可用 Alpha（REQ-037 / 038） | Should | 是（下一步最优先） | 解决“资料放不进去、备份不出来”两个真实使用阻塞 | 风险低；不引重依赖 |
...
| Phase2A 个人知识组织（REQ-012 / 026 / 025） | Could | 否（P1.5 后） | 资料多起来后需要标签、内链和轻量条目避免变乱 | 需补设计与迁移；优先级高于 AI 润色 |
| Phase2B 团队 MVP（REQ-014 / 013 / 024，后续 015..017） | Could | 否（个人可用后） | 小团队协作和写作辅助增强 | 需 Phase2 重新确认范围 / 进入 / 退出标准 |
...
| Phase1.5A（下一步） | `[P1]` · REQ-037、REQ-038 | **个人可用 Alpha** | 候选·待编码 | Phase1 Demo closure 已完成；Sprint-0′ 框架补课不阻塞 | 批量 / 文件夹 `.md/.txt` 入库可搜可问答；单文档 `.md` 与空间 ZIP 可导出备份；权限过滤正确；产品红线未破坏 |
...
| Phase2A（个人知识组织） | `[P2]` · REQ-026、REQ-012、REQ-025 | **个人知识组织** | 建议重排·待设计 | Phase1.5A/B 使真实资料量上来；06/07/09 补最小契约 | 文档可用内链 / 反链互联，可用标签组织，可快速录入轻量条目；适合个人长期管理资料 |
...
### Phase2A —— 个人知识组织（功能范围 `[P2]` · 交付物形态 **个人知识组织**）
...
- **功能范围 `[P2]`**：REQ-014 AI 润色 + 写作引用；REQ-013 时间轴视图 + REQ-024 时间轴密度热条作为候选；REQ-015 / 016 / 017 延后确认。
- **交付物形态 MVP**：可真实上线给小团队试用，补齐 Phase1 Demo 缺的生产要素，而非演示原型。
- **进入标准**：Phase2A 稳定；`04/05` Phase2 设计补强、tech-env-eval、`06/07/09` MVP 级契约齐备。
- **退出标准**：团队使用场景有真实验收 TC；AI 润色的数据外发风险可接受或可 Mock 降级；产品红线未被破坏。
...
| REQ-012 | F-007 | 标签视图 | `[P2]` | 个人知识组织 | Phase2A 骨架，启动时细化 |
| REQ-013 | F-008 | 时间轴视图 / 关联图视图 | `[P2]` | 团队 MVP / 后续候选 | Phase2B 候选，先时间轴后关联图 |
| REQ-014 | F-008 | AI 润色 + 写作侧边栏引用 | `[P2]` | 团队 MVP | Phase2B 骨架，需数据外发风险确认 |
...
| REQ-024 | F-008 | 时间轴密度热条 | `[P2]` | 团队 MVP / 后续候选 | Phase2B 候选，依赖时间轴 |
| REQ-025 | F-007 | 快速录入索引条目 | `[P2]` | 个人知识组织 | Phase2A 骨架，可选排在标签 / 内链之后 |
| REQ-026 | F-007 | 内部链接 + 反向链接 | `[P2]` | 个人知识组织 | Phase2A 优先，资料量上来后高 ROI |
...
| REQ-037 | F-011 | 批量 / 文件夹导入（`.md` / `.txt`） | `[P1]` | 个人可用 Alpha | P1.5A 候选·待编码 |
| REQ-038 | F-011 | 单文档 `.md` + 空间 ZIP 导出 | `[P1]` | 个人可用 Alpha | P1.5A 候选·待编码 |
