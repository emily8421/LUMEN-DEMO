...
- **功能范围 `[P2]`**：REQ-014 AI 润色 + 写作引用（**首批核心**）；REQ-013 时间轴视图 + REQ-024 时间轴密度热条（**紧随的第二 slice**，从零补设计）；REQ-039 文档目录树（**第三 slice 候选**，folder-tree，待 FT-C-* 确认 + 立项编码）；REQ-015 / 016 / 017 延后确认。
