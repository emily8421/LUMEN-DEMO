...
- **功能范围 `[P2]`**：REQ-014 AI 润色 + 写作引用（**首批核心**）；REQ-013 时间轴视图 + REQ-024 时间轴密度热条（**紧随的第二 slice**，已设计·候选 A 实时聚合 + TL-C-001..008 已确认）；REQ-039 文档目录树（**第三 slice 候选**，folder-tree，待 FT-C-* 确认 + 立项编码）；REQ-015 / 016 / 017 延后确认。
...
| REQ-013 | F-008 | 时间轴视图 / 关联图视图 | `[P2]` | 团队 MVP | **Phase2B 首批·第二 slice（紧随 REQ-014）**，先时间轴后关联图；**已设计（候选 A 实时聚合不建表，TL-C-001..008 已确认，见 `docs/design/timeline.md`）** |
...
| REQ-024 | F-008 | 时间轴密度热条 | `[P2]` | 团队 MVP | **Phase2B 首批·第二 slice（依赖 REQ-013）**；**已设计（候选 A，4 档色阶 + 事件数密度，TL-C-001..008 已确认）** |
