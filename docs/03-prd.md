...
| 最后更新 | 2026-07-11（Phase2 MVP 范围确认：核心 5 项） |
...
| Phase2（优化扩展） | `[P2]` · MVP 必做 REQ-012/014/025/026/027（+ 可选 013/024） | **MVP** | 范围已确认（核心 5 项） | Phase1 文档/权限/检索底座已实现；04/05 Phase2 设计补强 + tech-env-eval + 06/07 契约齐备（见 research 评估） | 核心 5 项 MVP 可真实上线 + 验收 TC（09 Phase2 用例，启动时细化）；REQ-009/010 真实化按 MVP 口径评估；产品红线未破坏 |
...
- **功能范围 `[P2]`（MVP 必做，2026-07-11 确认）**：REQ-012 标签视图、REQ-026 内部链接 + 反向链接、REQ-014 AI 润色 + 写作引用、REQ-025 快速录入索引条目、REQ-027 单文档导出 PDF（核心 5 项；依据 `docs/research/2026-07-11-phase2-mvp-scope-recommendation.md`）。
...
- **进入标准**：Phase1 文档 / 权限 / 检索底座已实现；`04/05` Phase2 设计补强（MOD-006/007 + Phase2 ADR + `05` RG-006..）、Phase2 tech-env-eval（Go / Conditional-Go）、`06/07` MVP 级契约齐备。
- **退出标准（MVP 上线验收）**：核心 5 项可真实上线且有验收 TC（`09` Phase2 用例，启动时细化）；REQ-009/010 真实化（真实 Word / PDF、OCR）按 MVP 口径纳入评估；产品红线未被破坏。
...
|---|---|---|---|---|---|
| PRD-C-001 | 是否正式进入 Phase2 | **范围已确认（核心 5 项 MVP，见 §3）**；进入仍需 04/05 Phase2 设计补强 + tech-env-eval + 06/07 契约 + 复跑升级评估，再切指针 | `ai/project-rules.md` §1 当前阶段仍为 Phase1；Phase2 MVP 范围见 §3 | 直接进入 Phase2 | 会跳过 Phase2 进入门禁；阻塞 Phase 指针变更 |
