...
| response_model/codegen | CQ-P1-006（64 endpoint/0 response_model，类型双写；实为 65，漏 `config_router`） | FastAPI `response_model` + OpenAPI → 前端 codegen / schema diff | 后端加字段前端自动同步 → R3 + R5（后端契约 + CI schema diff 已闭环；前端 codegen Slice A 已落地·批26 v3.8.23 / Slice B 待续） |
...
| 事务 UoW | 3 | ⏳ | — | P1 |
| response_model/codegen | 3 | ✅ 已闭环（Sprint-41 / task-048 / 62 JSON 端点 response_model + OpenAPI 快照 + CI schema-diff required，v3.8.13） | — | P1（前端 codegen Slice A 已落地·批26 v3.8.23 / Slice B 待续） |
