...
| response_model/codegen | CQ-P1-006（64 endpoint/0 response_model，类型双写；实为 65，漏 `config_router`） | FastAPI `response_model` + OpenAPI → 前端 codegen / schema diff | 后端加字段前端自动同步 → R3 + R5（后端契约 + CI schema diff 已闭环；前端 codegen Slice A / B-1 / B-2 已落地·批26-28 v3.8.23-25 / B-3·B-4 待续） |
...
| 事务 UoW | 3 | ⏳ | — | P1 |
| response_model/codegen | 3 | ✅ 已闭环（Sprint-41 / task-048 / 62 JSON 端点 response_model + OpenAPI 快照 + CI schema-diff required，v3.8.13） | — | P1（前端 codegen Slice A / B-1 / B-2 已落地·批26-28 v3.8.23-25 / B-3·B-4 待续） |
...
| 配置集中 | 3 | ✅ 已闭环（Sprint-44 / task-051 / PR #156，v3.8.16） | PR #156 `7388ff6` | P2（pydantic-settings 集中 + 生产弱默认 key fail-closed；llm_adapter 动态命名 / embedding 环境注入显式豁免） |
| 日志统一 | 3 | ✅ 已闭环（Sprint-43 / task-050 / PR #155，v3.8.15） | PR #155 `edb5df1` | P2（禁 print 用 ruff T20 落地，`scripts/` smoke / CLI 豁免） |
| integration 全量入 CI gate | 2/3 | ✅ 已落地（维护态批11 / v3.8.8） | PR | 独立 `backend-integration` job（pgvector 服务容器 + fail-closed 预检 + 48 用例），闭环 docs/05 §4.2.4「另议」；免费版私有仓库无分支保护未强制 required；merge 前人工核对 checks 绿 |
...
- **2026-08-13 追加（维护态批18 / v3.8.15）**：轨道 3 P2 第一项 `日志统一`（CQ-P2-002）全闭环——`print` 债清零 + 静默吞收口 + ruff `T20` 禁 print（ratchet）：`pg_repository.py:327` embedding 降级 `print` → `logger.warning`（后端生产 print 债清零）；`service/document.py` 索引回填失败、`service/rag.py` RAG / 通用对话 LLM 降级 ×2、`api/auth.py` 登录 `SpaceAccessError` 回退——补 `logger.warning` 记原因；`ruff.toml` select 加 `T20`（flake8-print）禁 print（`scripts/` smoke / CLI stdout 豁免）。验证：mypy 0 error（57 files）+ ruff passed（含 T20）+ 默认 pytest 313 passed / 49 deselected 零回归 + grep 复核 backend/ 无 print 残留 + T20 负向探针。PR #155 squash merge main `edb5df1`。详见 `tasks/task-050-log-unification.md`。
- **2026-08-13 追加（维护态批19 / v3.8.16）**：轨道 3 P2 第二项 `配置集中 + secret 校验`（CQ-P2-003）全闭环——新 `backend/config.py`（pydantic-settings `Settings`：`lumen_env`/`database_url`/`demo_token_key` + `get_settings()` 单例 + `validate_runtime_secrets`）；`db.py`/`main.py`/`auth.py` 三处静态 env 收敛（`llm_adapter` 动态命名配置、`embedding` 环境注入显式豁免）；`main.py` lifespan 生产 fail-closed 校验弱默认 signing key（仅 `LUMEN_ENV=production`，test/本地/demo 豁免）；新依赖 `pydantic-settings==2.15.0`（已确认）。验证：mypy 0 error（58 files）+ ruff passed + 默认 pytest 321 passed / 49 deselected（+8 config 用例）零回归 + secret 校验单测 8/8。PR #156 squash merge main `7388ff6`。详见 `tasks/task-051-config-centralization.md`。
- **2026-08-13 追加（维护态批20 / v3.8.17）**：轨道 3 P2 第三项 `前端 ratchet + App 减压`（CQ-P1-008）全闭环——`App.tsx` 546→360 拆三 shell（`AuthShell` / `WorkspaceShell` / `OverlayShell`，App 只留状态编排 + 装配）；新 `scripts/check-frontend-file-size.mjs`（Node 零依赖，`wc -l` 口径）+ `frontend/.file-size-baseline.json`（19 个超限基线）+ `package.json` `check:file-size` + CI project-check step（拦新增超限 / 超限膨胀）。验证：lint 0 problem + build 304 modules（+3 shell）+ file-size OK + 负向探针（260 行临时文件 fail）。PR #157 squash merge main `7b5444f`。详见 `tasks/task-052-frontend-ratchet-app-shell.md`。
- **2026-08-13 追加（维护态批21 / v3.8.18）**：轨道3 P2 剩余候选 E4 `前端文件膨胀拆分` Slice A 全闭环——`check-frontend-file-size.mjs` 分层阈值对齐 docs/05 §4.1（`.css` / `App.tsx` 主应用入口→300，`.ts/.tsx`→250，原统一 250 与文档口径不一致）；`App.tsx` 359→90（新 `app/useAppShellState.ts` 57 行 UI/布局 + 弹窗 state + 新 `app/useAppState.ts` 339 行域 hook 编排 17 个 + cross-cutting 回调 + 3 effects，依赖顺序原样搬移）+ App 只剩装配；useAppState 因跨域回调与域 hook 循环依赖登记基线（App 编排宿主例外，用户确认）；baseline 重生成（19→18，App.tsx / ai-assistant.css 出基线）。验证：lint 0 problem + build 306 modules（+2 hook）+ file-size OK + 负向探针（280 行临时文件 fail）+ 浏览器 smoke（demo + headless Edge 渲染登录页无运行时错误）。PR #158 squash merge main `ee95523`。详见 `tasks/task-053-frontend-app-state-split.md`。
- **2026-08-13 追加（维护态批22 / v3.8.19）**：轨道3 P2 剩余候选 E4 `前端文件膨胀拆分` Slice B 全闭环——4 个超限 `.ts` 按风险低→高拆：`local-vault-fs` 310→`local-vault-walk`(67)/`local-vault-idb`(152)/`local-vault-fs`(94) 三纯工具；`useFolders` 300→`folder-utils`(46)+`useFolderInlineEdit`(122)+主 hook(202)；`useDocuments` 335→`useDocumentSideData`(110)+`download-actions`(30)+主 hook(286 登记，核心 CRUD 例外)；`useLocalVaultMount` 442→`local-vault-tree`(32)/`local-vault-types`(60)/`useLocalVaultEditor`(184)+主 hook(236)；baseline 18→15。验证：lint 0 + tsc 0 + build 314 modules（+9 文件）+ file-size OK + 负向探针（280 行 fail）+ 浏览器 smoke（headless Edge 登录页无错误）。PR #159 squash merge main `bad7e4c`。详见 `tasks/task-054-frontend-hooks-split.md`。
- **2026-08-13 追加（维护态批23 / v3.8.20）**：轨道3 P2 剩余候选 E4 `前端文件膨胀拆分` Slice C 全闭环——4 个超限 CSS（阈值 300）全拆：`workspace` 722→`tree.css`(189)/`editor.css`(259)/`workspace.css`(281)；`local-mount` 589→`local-mount-pane.css`(156)/`local-mount-tree.css`(236)/`local-doc-preview.css`(127)/`local-mount-menu.css`(75)；`layout` 458→`topbar.css`(279)/`workspace-layout.css`(184)；`onboarding` 317→`onboarding.css`(167)+`welcome.css` 吸收 checklist(230)；`main.tsx` import 原位重排（层叠约束保持，responsive.css 前）。验证：build 320 modules + CSS bundle 65.60 kB（= 拆分前，内容无丢失）+ file-size OK（11 基线）+ 负向探针（280 行 fail）+ 浏览器 smoke（headless Edge 登录页渲染正常）。PR #160 squash merge main `071a73b`。详见 `tasks/task-055-frontend-css-split.md`。
- **2026-08-13 追加（维护态批24 / v3.8.21）**：轨道3 P2 剩余候选 E4 `前端文件膨胀拆分` Slice D 全闭环——6 个超限 .tsx 组件拆分 + 共享 `tree/` 抽取：`FolderTree` 569→`app/folder-tree/`（FolderTree+FolderNode+DocumentRow）+ 共享 `app/tree/TreeInlineEditor`（FolderInlineEditor/CategoryInlineEditor 100% 重复）+`useTreeMenuDismiss`（FolderNode/DocumentRow/CategoryNode 三处逐字相同菜单关闭 useEffect，消除 ~170 行重复）；`TermCategoryTree` 405→`app/term-category-tree/`（TermCategoryTree+CategoryNode）复用共享 tree/（不做泛化递归树）；`TopBar` 274→`app/topbar/`（TopBar+HelpPopover+UserMenu+PaneToggles，help state 内聚）；`ContextPane` 366→`app/context-pane/`（ContextPane+FolderTreeHeader+TermsContextPane+ContextInfoList，搜索/问答 info-list DRY）；`ImportFeature` 335→`features/import-files.ts`（纯文件收集工具）+`features/import/`（ImportDropZone+ImportResultsList）+主文件；`DocumentInspectorFeature` 266→`features/document-inspector/`（主文件+InspectorVersionsTab+InspectorLinksTab+InspectorTagsTab）；baseline 11→5。验证：lint 0 + tsc 0 + build 337 modules（+17 文件，无新依赖）+ file-size OK（5 基线）+ 负向探针（280 行 fail）+ 浏览器 smoke（真实登录 alice → 工作区渲染全部拆分组件 + 文件夹右键菜单/内联编辑器/Esc 关闭/折叠 + 文档行菜单 + 文档侧栏四 tab，无运行时/JS 错误）。PR #161 squash merge main `8ea9031`。详见 `tasks/task-056-frontend-component-split.md`。
- **2026-08-13 追加（维护态批25 / v3.8.22）**：轨道3 P2 剩余候选 E4 `前端文件膨胀拆分` Slice E 全闭环（E4 收口 / GOV-015）——3 个超限 .tsx 纯位移拆分：`WorkspaceMain` 279→248（抽 `app/WorkspaceMainDocuments`）；`DocumentsFeature` 485→234（`features/documents/`：useSplitDragController/useEditorUndoStack/MarkdownToolbar/DocumentEditorForm/DocumentPreviewPane；textareaRef 三方共享内聚于 DocumentEditorForm）；`LocalMountPane` 563→140（`features/local-mount/`：useLocalMountImport/LocalMountHeader/LocalMountImportBar/LocalMountTreeView 302→242/LocalMountContextMenus/useInlineEdit/LocalMountInlineInput；TreeView 二次拆菜单/输入）；baseline 5→2（E4 收口：Slice A-E baseline 19→2，剩 useAppState/useDocuments 两核心编排例外）。验证：lint 0 + tsc 0 + build 350 modules（CSS 65.60 kB 不变）+ file-size OK（2 基线）+ 负向探针 + 浏览器 smoke（smoke-vault-local-mount OK）+ CI PR #162 8 job 全绿。PR #162 squash merge main `9f5c967`。详见 `tasks/task-057-frontend-component-split-slice-e.md`。
- **2026-08-13 追加（维护态批26 / v3.8.23）**：轨道3 P1 `前端 codegen`（CQ-P1-006 后续）Slice A 试点——`openapi-typescript@^7.13.0`（devDep）+ `gen:api` 脚本从 `openapi/openapi.json` 生成 `frontend/src/api/generated.ts`（入库，避开 `--immutable` v7 bug #1368）；`tags.ts` 混合接入（TagView `Omit+status` narrow / DocumentTagView·TagLinkView 零差异 alias / ListEnvelope 手写保留）；CI `frontend-schema-diff` advisory job（复用 schema-diff 的 `git diff --exit-code`）；`check-frontend-file-size.mjs` 豁免 codegen 产物。**接入策略=混合接入**（Explore 评估揭示 openapi 几乎无 enum，全量替换丢 ~12 处 union narrow；documents Summary|Detail 合并留 Slice B 单独批）。验证：gen 幂等 drift 0 + lint 0 + build 350 modules 0 error（零回归）+ file-size OK + tags 消费方 tsc 零回归；浏览器 smoke 留 Slice B（纯类型改动 + build 强保证）。CI PR #165 9 job 全绿（含新 frontend-schema-diff 首跑）。PR #165 squash merge main `1a06f8d`。详见 `tasks/task-058-frontend-codegen.md`。
