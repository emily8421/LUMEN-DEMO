...
| 文件 | 类型 | 状态 | 采纳 / 后续 |
|---|---|---|---|
| `2026-08-19-frontend-code-evaluation.md` | 前端代码质量评估复核（170 文件走读 + 初评四道质量门记录 + 可访问性 / 刷新链静态复核；2026-08-19 二次评审后修订：+P8 client 非 JSON 回退、R5 计数 5→7） | 已落盘；**AI 评审 · 待人工确认采纳范围** | 基线质量门记录全绿；复核新增 FE-A11Y-1/2（状态播报、tabs、弹层焦点）、FE-RES-1（ErrorBoundary）、FE-RACE-1（刷新结果归属）、FE-ERR-1（client 非 JSON 回退）与 FE-TEST-1（需确认 Vitest）。`AbortController` 基础能力已存在于 client，不改基础设施。执行候选见 `2026-08-19-frontend-remediation-plan.md` |
| `2026-08-19-frontend-remediation-plan.md` | 前端修复候选实施计划（评估结论到独立工作包的转译；二次评审后修订：FEP-05 补修改范围与 FEP-04 协同触发、FEP-01 验收补验证手段、FEP-03 补错误留痕；外部反馈再修订：FEP-01 补双 panel 常驻 DOM + `[hidden]` 样式覆盖坑） | 已落盘；**候选计划 · 未授权编码** | 五个工作包按无障碍、交互韧性、测试基础设施、刷新竞态、维护债拆分；FE-ERR-1 随下次触及 client.ts 合并；仅在用户批准具体工作包后，才回填 `08` / `09` 并建立任务单 |
...
| `2026-08-18-code-directory-review.md` | 代码目录逐目录评审（对照 05 §4.1.0 五条依据 + §4.1 阈值实测；backend 四层 / frontend 五区 / 横切目录；改进建议 R1-R4） | 已落盘；**AI 评审 · 待人工确认采纳范围**（R1/R2/R3 均已完成） | 整体判级健康，无 ❌ 级问题；R1 已落地（v3.13.0）；R2 god object 已收敛（Sprint-59：8 域子 Protocol）；R3 local-vault 六模块已迁 features/local-mount（Sprint-60）；R4 scripts 分目录维持不动、触发线 ≥60 已登记；学科出处见 references/software-engineering-basics.md §3 |
