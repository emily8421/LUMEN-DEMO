...
| `2026-08-18-code-directory-review.md` | 代码目录逐目录评审（对照 05 §4.1.0 五条依据 + §4.1 阈值实测；backend 四层 / frontend 五区 / 横切目录；改进建议 R1-R4） | 已落盘；**AI 评审 · 待人工确认采纳范围**（R1 已随 FG-C2 完成；R2 已完成 Sprint-59） | 整体判级健康，无 ❌ 级问题；~~核心短板 = 前端有 ratchet 后端无（R1 补齐）~~ R1 已落地（v3.13.0）；R2 god object 已收敛（Sprint-59：8 域子 Protocol + 聚合父契约）；R3 local-vault 归属 / R4 scripts 分目录，仍登记为债不强制回改；学科出处见 references/software-engineering-basics.md §3 |
