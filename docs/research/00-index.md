...
| 文件 | 类型 | 状态 | 采纳 / 后续 |
|---|---|---|---|
| `2026-08-17-docs-system-oo-methodology-redesign.md` | 文档体系适配 OO 开发方法方案（图纸驱动编码 + 00-09 组织 + 详细设计分类） | 已落盘；**方案一稿确认，实施分步（先补详细类图 DIAG-CLS → 概设类图 → 组织微调 → 回流模板）** | 图纸体系缺口=缺类图族（概设类图 + 详细类图），图纸驱动编码链：用例图→AC/TC、领域模型→06 表、详细类图→类代码、活动/状态图→流程代码 |
