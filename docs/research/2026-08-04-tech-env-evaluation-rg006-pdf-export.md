...
| 不代表 | API-019 / 产品级 PDF 导出尚未实现；TC-P1-017 仍需 Sprint-18 编码后闭环 |
...
|---|---|---|---|---|
| RG-006 | PDF 库可安装 / 导入；中文字体可用；能生成中文样例并渲染检查 | 依赖安装记录、smoke 脚本、PDF + PNG + 文本抽取结果、人工视觉检查 | **Go** | 无；API-019 产品实现仍待 Sprint-18 |

## 7. 风险与降级

| Risk-ID | 优先级 | 风险 | 当前状态 | 降级 / 处理 |
|---|---|---|---|---|
| RISK-P1-006 | P1 | PDF 导出库未验证 | **已关闭**：ReportLab 中文样例通过 | Sprint-18 可进入编码；运行时缺库 / 字体缺失时返回 5030 |
| PDF-IMPL-001 | P1 | Markdown 子集映射未实现 | 待 Sprint-18 | 首版只承诺标题、段落、列表、表格、引用、页眉页脚 |
| PDF-IMPL-002 | P1 | 导出产物权限 / 清理策略未实现 | 待 Sprint-18 | 复用文档可读权限；产物继承源权限；临时文件需过期清理或流式响应 |

## 8. 结论与下一步

RG-006 结论为 **Go**。可以进入 Sprint-18 单文档 PDF 导出编码，但必须明确：当前只关闭技术环境门禁，不代表 PDF 导出功能已完成。

下一步建议：
- 启动 Sprint-18，按 API-019 实现 `POST /api/export-pdf`。
- 后端使用 ReportLab 生成 PDF，缺依赖 / 缺字体 / 渲染失败时返回 5030。
- 补 TC-P1-017：权限、版本绑定、中文导出、失败态、产物访问、人工 PDF 样例。
