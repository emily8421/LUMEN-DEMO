...
| REQ-018 | Obsidian Vault 兼容：同一文件夹来源可选择“导入数据库”或“仅本地挂载”；导入后成为 LUMEN 正式文档并参与权限 / 搜索 / RAG；仅挂载内容保持个人 / 当前设备可见，不默认进入团队空间、后端 RAG 或共享权限链 | U-21 | 待技术验证（浏览器 / 桌面本地目录授权、只读/可写策略、本地索引、增量同步、删除/重命名冲突、隐私边界） | [愿景] | 已确认方向·待技术验证 |
...
| U-21 | REQ-018 | [愿景] | Obsidian Vault 兼容（导入数据库 / 仅本地挂载双模式） | 已确认方向·待技术验证 | 仅挂载不默认写入 LUMEN DB，不进入团队共享 |
