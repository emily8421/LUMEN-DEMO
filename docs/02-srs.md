...
| 最后更新 | 2026-07-11（REQ 主表覆盖 001..036，对齐 doc-standards） |
...
| REQ-038 | 文档 / 空间导出：单文档下载 `.md`；空间打包 ZIP 导出所有可见文档 `.md`（权限过滤） | 新增（U-ID 待补） | 文档详情可下载 `.md`；空间导出 ZIP 含可见文档 | [P1] | P1.5-候选·待编码 |
| REQ-012 | 标签视图：跨目录聚合同标签文档 | U-13 | 待该阶段细化（标签模型、聚合规则） | [P2] | 骨架 |
...
| REQ-027 | 单文档导出 PDF（Markdown → PDF，含中文排版） | U-33 | 选型 weasyprint/reportlab + 中文验证（RG-006）；封面 / 页眉页脚 / 排版 | [P1] | P1.5-候选·待 RG-006（从 Phase2 提前） |
...
> 阅读索引：上方 REQ 主表已覆盖全部 REQ-001..036 的「系统需求 / 来源 U-ID / 可验证口径 / 初步阶段 / 状态」，不再以分组 bullet 双写（避免与主表漂移）。P1 REQ 可验证口径见上表；P2 / 愿景 REQ 保留骨架，可验证口径写「待该阶段细化 / 待技术验证」，升阶段时再细化。
...
| U-33 | REQ-027 | [P1] | 单文档导出 PDF | P1.5-候选·待 RG-006（从 Phase2 提前） | — |
