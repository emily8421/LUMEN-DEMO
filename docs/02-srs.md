...
| REQ-013 | 时间轴视图 / 关联图视图 | U-14 / U-15 | Phase2B 第二 slice·已设计：候选 A 实时聚合（lumen_documents/tag_links/doc_links 的 created_at）；布局=密度热条+事件列表+列表逃生舱（PG-P2-008）；关联图仍候选 | [P2] | Phase2B·已设计（TC-P2-TL-001 待实现） |
...
| REQ-024 | 时间轴密度热条 | U-30 | Phase2B 第二 slice·已设计：4 档色阶（0 无 / 1 低 / 2 中 / 3 高）；密度按事件数 + 渲染去重；日窗口（>180 天切周） *(v18)* | [P2] | Phase2B·已设计（TC-P2-TL-001 待实现） |
