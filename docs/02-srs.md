...
| REQ-013 | **主题时间线（REQ-013a·按关键词/标签生成事件时间线）+ 关联图视图（REQ-013b）** | U-14 / U-15 | **REQ-013a（主题时间线，U-14）Phase2B 第二 slice·已设计**：关键词/标签驱动（API-033 `q`/`tag_ids`），候选 A 实时聚合 + 标题 ILIKE / chunk.ts_vector 命中 + actor（linked=null）+ 密度热条（含量化 ratio）+ 列表逃生舱（PG-P2-008）；不传 `q`=空间总览。**REQ-013b（关联图，U-15）愿景**，待主题时间线落地后评估（见 `docs/design/timeline.md` §9 重定位） | [P2] | REQ-013a Phase2B·已设计（TC-P2-TL-001 待实现，编码移至 folder-tree 后）；REQ-013b 愿景骨架 |
