...
| Sprint-28（Phase2D·角色分层 + 用户管理 + 团队空间加入·**已立项**） | REQ-045 全局角色分层（`lumen_users.role` admin/member）+ REQ-046 用户管理后台（admin 域：用户列表 / 过滤 / 改角色 / 禁用启用）+ REQ-047 团队空间加入机制（space 域成员 CRUD：按 email 添加 / 改空间角色 / 移除 + 空间设置 UI） | 045/046/047 | 03 Phase2D、design/accounts-auth §18、02 REQ-045..047、01 U-50..52 + AC-P2-ACC-004..006、09 TC-P2-ACC-002、tasks/task-040 | backend（migration 016 `lumen_users.role` + admin 域 user API + space 域 member API + 用户搜索）+ frontend（用户管理页 + 空间设置成员管理）；零新依赖 | TC-P2-ACC-002 | **已立项（2026-08-07）**：REQ/U/AC/TC 契约落盘（02/01/03/08/09/accounts-auth §18/task-040）；C-ROLE-001..004 已确认（单列角色 / 最小集 / 按 email 添加 / seed alice=admin）；编码 Sprint 待启动 | tasks/task-040-role-user-admin.md |
