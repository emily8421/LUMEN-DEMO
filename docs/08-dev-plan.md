...
| Sprint-23C（Phase2C·模式 B 编码·待启动） | REQ-018 模式 B 浏览器 File System Access 编码：句柄持久化 + 本地索引搜索 + 左侧分区 + 按需导入 | 018 | 03 Phase2C、04 Flow-010、05 TCD-011 / RG-009 / TC-P2-VAULT-001、06 `lumen_vault_mounts`（migration 014）、07 模式 B 零后端 API、design/ingestion Flow-D-014、design/frontend-interaction CMP-P2-TREE / PATH-P2-008 | 接 `frontend/src/`（ContextPane 本地挂载分区 + useLocalVaultMount hook）；不引第三方依赖 | TC-P2-VAULT-001 | Phase2C·待启动（设计层已就绪，编码范围待 Sprint 启动确认） | — |
