...
| Sprint-57（维护态批32） | 2026-08-14 | CI 省额度拆分——project-check 拆 3 workflow + paths 路径过滤（方案 1+2，非功能·不改 Phase） | `project-check.yml` 改造为轻量通用检查（whitespace / 版本一致性 / sync boundary / file-size ratchet，**总是触发** ~0.1min）+ 新增 `backend-ci.yml`（unit / integration(PG) / mypy / ruff / schema-diff 5 job，paths：`backend/**`、`tests/backend/**`、`openapi/**`、`requirements*`、`mypy.ini`、`ruff.toml`、`scripts/export-openapi.py`）+ 新增 `frontend-ci.yml`（build / eslint / codegen drift 3 job，paths：`frontend/**`、`openapi/**`）；**9 job 全迁移无丢失**；**push main 同 paths 过滤**（方案 2：合并 main 只跑实际改动域，不再 9 job 全量重跑）；`openapi/**` 同时触发两端防 `generated.ts` 失配；附计费分析 / 模板回流判断 / 自托管评估 3 份 research 决策留痕 | PR #172（分支 `chore/ci-split-paths-filter`，commit `bc185dc`→`239f3ca` 共 4 个；**挂起等额度重置**，未 merge） | 本地 PyYAML 语法 + 结构 + 9 job 迁移完整性校验全过；**真实触发行为待额度恢复后确认**（首验：纯文档批 PR 只跑 project-check ~0.1min，见 `docs/research/2026-08-14-github-actions-billing-analysis.md` §6） | 阻塞：GitHub 账号 2000min/月免费额度耗尽（2026-08-14 实测，未绑卡即停）；已决策**暂缓自托管 runner、挂起等额度重置**（约 09-01，见 `docs/research/2026-08-14-self-hosted-runner-evaluation.md`）；模板回流判断：**暂不回流攒实证**（merge 后 1-2 批实证 paths 生效再起草 `_proposals/TEMPLATE-UPGRADE-ci-budget-paths-filter.md`）；纯 CI 配置不改对外 API / DB / 依赖；不 bump VERSION（v3.8.27） | §2 / §5 |
