...
| Sprint-30（维护态批5） | 2026-08-08 | REQ-050 / REQ-051（使用反馈⑧⑩） | **REQ-050** admin 用户空间可见性（API-054 admin 只读接口 `GET /api/admin/users/{user_id}/spaces` + `GET /api/spaces` admin 全空间分支 + 前端用户详情抽屉即时操作）；**REQ-051** 登录密码小眼睛 + 登录 / 注册标准优化（错误内联 + 回车 + loading）+ 忘记密码（API-055/056 reset request/confirm + migration 018 `lumen_users` reset 字段 + 重置吊销全部 session，demo token 写日志） | 编码中（待提交，目标 v3.7.0） | 后端 unittest + 前端 build + `scripts/smoke-batch5-auth-admin-browser.mjs`（待补） | demo 无 SMTP → reset token 降级写日志；不实现邮箱验证 / OAuth / 邀请码 / 移除用户（Sprint-28 候选） | §2 / §5（TC-P2-ACC-003 / TC-P2-AUTH-002） |
