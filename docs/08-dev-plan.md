...
| Sprint-23A（候选·导入规模修复） | 1000+ 文件夹导入分批上传 + 汇总结果 | 037/039 | 07 API-029、design/ingestion Flow-006、09 TC-P1-015 | frontend `api/imports.ts` + `app/useImport.ts` + `features/ImportFeature.tsx` | TC-P1-015 回归 + 1000+ 文件夹 smoke | 已编码（PR #104，2026-08-04）：前端分批 50/批顺序上传 + 逐批失败隔离 + 汇总，失败项全显/成功项截断；纯前端、不改后端 API；build + 后端 11 测试绿；待合并 + 1000+ 文件夹 smoke | — |
| Sprint-23B（愿景 RG·Vault 兼容） | Obsidian Vault 兼容技术验证：导入数据库 / 仅本地挂载双模式 + 左侧文件管理器上下分区 | 018/037/039 | 02 REQ-018、04 ADR-011、05 TCD-011 / RG-009、06 `lumen_vault_mounts` 骨架、07 REQ-018 接口口径、design/ingestion Flow-D-014、design/frontend-interaction PG-P2-002 / CMP-P2-TREE | PoC / 文档 / 原型优先；如进入编码需确认浏览器 File System Access / 桌面客户端路线 | TC-VISION-VAULT-001（待定义） | 愿景候选·已确认方向·待技术验证；不得直接作为当前 Phase 编码任务 | — |
...
| Sprint-23B（愿景 RG·Vault 兼容） | Sprint-23A 或现有导入可用；用户确认双模式方向 | 浏览器本地目录授权 / 持久化、隐私边界、仅本地索引、文件变更同步、桌面运行形态未验证 | 可与 Word/PDF 解析 RG 并行 | 先做 PoC / 原型 / 技术评估；确认后再决定是否进入产品 Sprint | M16 个人本地 Vault 连接器可行性明确 |
