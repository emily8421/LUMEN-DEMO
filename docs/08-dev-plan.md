...
| Sprint-33（维护态批8） | 2026-08-12 | P1 repository 契约（CQ-P1-002，非功能·不改 Phase） | **Slice A** RepositoryProtocol 契约化（`backend/repository/protocol.py` typing.Protocol + @runtime_checkable + 98 方法 inspect 机器提取自 PgRepository + pg/demo 显式继承 + contract test 4 测试守护双实现一致）；Slice B（按域拆 god object）/ Slice C（service repository 参数注解）待续 | Slice A PR #139（`a17ea68`） | contract test 4 passed + 全量 301 passed 零回归（297→301）+ ruff 37 基线不增 | 纯契约化，不改运行时行为 / 对外 API / DB；为 CQ-P1-003 事务边界 UoW 提供 Protocol 基础；详见 `docs/research/2026-08-10-code-governance-rollout-plan.md` §4 轨道3 + §5 轨道C | §2 / §5 |
