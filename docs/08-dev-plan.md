...
> **候选·待确认**：方案已确认【单文档.md + 空间ZIP】。不碰 PDF 导出（REQ-027 留 Phase2）。
...
- 不做 PDF（留 REQ-027/Phase2）；不引重依赖（zipfile 标准库）。
