...
def register(repository, email: str, name: str, password: str) -> User:
...
def _derive_external_id(repository, email: str) -> str:
...
def resolve_session(repository, token: str) -> Session | None:
...
def refresh_session(repository, token: str) -> tuple[str, Session]:
...
def list_active_sessions(repository, user_id: int) -> list[Session]:
    return repository.list_sessions(user_id)


def revoke_session(repository, session_id: int, user_id: int) -> bool:
    return repository.revoke_session(session_id, user_id)


def update_session_space(repository, session_id: int, space_id: int) -> Session:
