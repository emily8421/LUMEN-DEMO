...
class ApiRouteTest(unittest.TestCase):
    ...
    def test_login_list_spaces_and_switch_space(self) -> None:
        ...
        login_response = login(LoginRequest(login_id="alice", password="demo-pass-1234", current_space_id=10))
        token = login_response["data"]["token"]
        authorization = f"Bearer {token}"
        from backend.service.auth_context import get_current_user
        ctx = get_current_user(authorization=authorization)

        spaces_response = list_spaces(ctx=ctx)
        ...
        # 契约变更（Sprint-26）：switch 不再重发 token，session 承载 current_space_id
        self.assertNotIn("token", switch_response["data"])
        switched_ctx = get_current_user(authorization=authorization)
        self.assertEqual(switched_ctx.current_space_id, 20)
    ...
    def test_document_crud_versions_and_restore(self) -> None:
        ...
        list_response = list_documents(ctx=ctx)
        detail_response = get_document_endpoint(document_id, ctx=ctx)
        versions_response = list_document_versions(document_id, ctx=ctx)
        restored_response = restore_document_version(document_id, 2, ctx=ctx)
    ...
    def test_import_api_rejects_unsupported_file_type(self) -> None:
        ...
        with self.assertRaises(HTTPException) as context:
            asyncio.run(import_file_endpoint(file=FakeUploadFile(), ctx=ctx))
    ...
    def test_search_api_returns_imported_text_hits(self) -> None:
        ...
        search_response = search_endpoint(q="degraded search", ctx=ctx)
    ...
    def test_search_api_returns_vector_semantic_hits(self) -> None:
        ...
        search_response = search_endpoint(q="响应速度和时延", ctx=ctx)
    ...
    def test_query_api_returns_not_found_without_sources(self) -> None:
        ...
        query_response = query_endpoint(QueryRequest(question="完全不存在的问题"), ctx=ctx)
    ...
    def test_created_document_is_searchable_and_queryable(self) -> None:
        ...
        search_response = search_endpoint(q="smoke search", ctx=ctx)
        query_response = query_endpoint(QueryRequest(question="场景联动触发延迟是多少？"), ctx=ctx)
    ...
    def test_terms_api_crud_and_rag_term_source(self) -> None:
        ...
        list_response = list_terms_endpoint(ctx=ctx)
        query_response = query_endpoint(QueryRequest(question="触发延迟是多少？"), ctx=ctx)
        updated_response = update_term_endpoint(
            term_id,
            TermWriteRequest(term="触发延迟", definition="更新后的空间定义", aliases=[], status="pending"),
            ctx=ctx,
        )
        delete_response = delete_term_endpoint(term_id, ctx=ctx)
