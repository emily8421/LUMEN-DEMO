...
- **维护态批18（Sprint-43，2026-08-13·CQ-P2-002）**：日志统一（轨道3 P2 第一项）——`print` 债清零 + 静默吞收口 + ruff `T20` 禁 print（ratchet）：`pg_repository.py` embedding 降级 `print` → `logger.warning`（后端生产 print 债清零）；`service/document.py` 索引回填失败、`service/rag.py` RAG / 通用对话 LLM 降级 ×2、`api/auth.py` 登录 `SpaceAccessError` 回退——补 `logger.warning` 记原因；`ruff.toml` select 加 `T20`（flake8-print）禁 print（`scripts/` smoke / CLI stdout 豁免）。验证：mypy 0 error（57 files）+ ruff passed（含 T20）+ 默认 pytest 313 passed / 49 deselected 零回归 + T20 负向探针；PR #155 squash merge main `edb5df1`，bump v3.8.15。实施口径 `tasks/task-050-log-unification.md`
- **维护态批19（Sprint-44，2026-08-13·CQ-P2-003）**：配置集中 + secret 校验（轨道3 P2 第二项）——新 `backend/config.py`（pydantic-settings `Settings`：`lumen_env`/`database_url`/`demo_token_key` + `get_settings()` 单例 + `validate_runtime_secrets` 生产 fail-closed）；`db.py` / `main.py` / `auth.py` 静态 env 收敛（`llm_adapter` 动态命名配置、`embedding` 环境注入显式豁免）；`main.py` lifespan 生产弱默认 signing key 拒绝启动（test / 本地开发 / demo 豁免）；新依赖 `pydantic-settings==2.15.0`（已确认）。验证：mypy 0 error（58 files）+ ruff passed + 默认 pytest 321 passed / 49 deselected（+8 config 用例）零回归 + secret 校验单测 8/8；PR #156 squash merge main `7388ff6`，bump v3.8.16。实施口径 `tasks/task-051-config-centralization.md`
- **维护态批20（Sprint-45，2026-08-13·CQ-P1-008）**：前端 ratchet + App 减压（轨道3 P2 第三项）——`App.tsx` 546→360 拆 `AuthShell` / `WorkspaceShell` / `OverlayShell` 三 shell（App 只留状态编排 + 装配）；新 `scripts/check-frontend-file-size.mjs` + `frontend/.file-size-baseline.json`（19 个超限基线）+ `package.json` `check:file-size` + CI project-check step（拦新增超限 / 超限膨胀，棘轮只进不退）。验证：lint 0 + build 304 modules + file-size OK + 负向探针；PR #157 squash merge main `7b5444f`，bump v3.8.17。实施口径 `tasks/task-052-frontend-ratchet-app-shell.md`
- **维护态批21（Sprint-46，2026-08-13·CQ-P1-008 候选 E4）**：前端文件膨胀拆分 Slice A——App 减压收口 + ratchet 分层阈值——`check-frontend-file-size.mjs` 按 docs/05 §4.1 分层阈值（`.css` / `App.tsx` 主应用入口→300，`.ts/.tsx`→250，原统一 250 与文档口径不一致）；`App.tsx` 359→90（新 `app/useAppShellState.ts` 57 行 UI/布局 + 局部弹窗/引导 state + 新 `app/useAppState.ts` 339 行域 hook 编排 17 个 + cross-cutting 回调 + effects，依赖顺序原样搬移）+ App 只剩装配；useAppState 因跨域回调与域 hook 循环依赖登记基线（App 编排宿主例外，用户确认）；baseline 重生成（19→18）。验证：lint 0 + build 306 modules + file-size OK + 负向探针 + 浏览器 smoke（headless Edge 登录页渲染无运行时错误）；PR #158 squash merge main `ee95523`，bump v3.8.18。实施口径 `tasks/task-053-frontend-app-state-split.md`
- **维护态批22（Sprint-47，2026-08-13·CQ-P1-008 候选 E4）**：前端文件膨胀拆分 Slice B——hooks/工具拆分——`local-vault-fs` 310→`local-vault-walk.ts`(67)/`local-vault-idb.ts`(152)/`local-vault-fs.ts`(94) 三纯工具；`useFolders` 300→`folder-utils.ts`(46)+`useFolderInlineEdit.ts`(122)+主 hook(202)；`useDocuments` 335→`useDocumentSideData.ts`(110)+`download-actions.ts`(30)+主 hook(286 登记，文档域核心 CRUD 例外)；`useLocalVaultMount` 442→`local-vault-tree.ts`(32)/`local-vault-types.ts`(60)/`useLocalVaultEditor.ts`(184)+主 hook(236)；baseline 18→15。验证：lint 0 + tsc 0 + build 314 modules + file-size OK + 负向探针 + 浏览器 smoke（headless Edge 登录页无错误）；PR #159 squash merge main `bad7e4c`，bump v3.8.19。实施口径 `tasks/task-054-frontend-hooks-split.md`
- **维护态批23（Sprint-48，2026-08-13·CQ-P1-008 候选 E4）**：前端文件膨胀拆分 Slice C——CSS 拆分（阈值 300）——`workspace` 722→`tree.css`(189)+`editor.css`(259)+`workspace.css`(281)；`local-mount` 589→`local-mount-pane.css`(156)+`local-mount-tree.css`(236)+`local-doc-preview.css`(127)+`local-mount-menu.css`(75)；`layout` 458→`topbar.css`(279)+`workspace-layout.css`(184)；`onboarding` 317→`onboarding.css`(167)+`welcome.css` 吸收 checklist(230)；`main.tsx` import 原位重排（层叠约束保持）；baseline 15→11（CSS 全出基线）。验证：build 320 modules + CSS bundle 65.60 kB（= 拆分前，内容无丢失）+ file-size OK + 负向探针 + 浏览器 smoke（headless Edge 登录页渲染正常）；PR #160 squash merge main `071a73b`，bump v3.8.20。实施口径 `tasks/task-055-frontend-css-split.md`
- **维护态批24（Sprint-49，2026-08-13·CQ-P1-008 候选 E4）**：前端文件膨胀拆分 Slice D——组件拆分（6 个超限 .tsx 组件拆分 + 共享 tree/ 抽取）：`FolderTree` 569→`app/folder-tree/`（FolderTree+FolderNode+DocumentRow）+ 共享 `app/tree/TreeInlineEditor`（FolderInlineEditor/CategoryInlineEditor 100% 重复）+`useTreeMenuDismiss`（三处逐字相同菜单关闭 useEffect，消除 ~170 行重复）；`TermCategoryTree` 405→`app/term-category-tree/`（TermCategoryTree+CategoryNode）复用共享 tree/（**不做**泛化递归树）；`TopBar` 274→`app/topbar/`（TopBar+HelpPopover+UserMenu+PaneToggles，help state 内聚）；`ContextPane` 366→`app/context-pane/`（ContextPane+FolderTreeHeader+TermsContextPane+ContextInfoList）；`ImportFeature` 335→`features/import-files.ts`+`features/import/`（ImportDropZone+ImportResultsList）；`DocumentInspectorFeature` 266→`features/document-inspector/`（主文件+InspectorVersionsTab+InspectorLinksTab+InspectorTagsTab）；baseline 11→5。验证：lint 0 + tsc 0 + build **337 modules**（+17 文件，无新依赖）+ file-size OK（5 基线）+ 负向探针 + 浏览器 smoke（真实登录 alice → 工作区渲染全部拆分组件 + 文件夹右键菜单/内联编辑器/Esc 关闭/折叠 + 文档行菜单 + 文档侧栏四 tab，无运行时/JS 错误）；CI PR #161 **8 job 全绿**；PR #161 squash merge main `8ea9031`，bump v3.8.21。实施口径 `tasks/task-056-frontend-component-split.md`
- **维护态批25（Sprint-50，2026-08-13·CQ-P1-008 候选 E4 收口 / GOV-015）**：前端文件膨胀拆分 Slice E——组件拆分·中风险 + baseline 收窄——3 个超限 .tsx 纯位移拆分：`WorkspaceMain` 279→248（抽 `app/WorkspaceMainDocuments.tsx`，documents 视图分支三元 LocalDocPreview/DocumentsFeature）；`DocumentsFeature` 485→234（→`features/documents/`：useSplitDragController + useEditorUndoStack + MarkdownToolbar + DocumentEditorForm + DocumentPreviewPane；textareaRef 三方共享内聚于 DocumentEditorForm——撤销/AI选区/MD插入）；`LocalMountPane` 563→140（→`features/local-mount/`：useLocalMountImport + LocalMountHeader + LocalMountImportBar + LocalMountTreeView（302→242）+ LocalMountContextMenus + useInlineEdit + LocalMountInlineInput；TreeView 二次拆右键菜单/内联输入）；baseline 5→2（E4 收口，剩 useAppState 339 / useDocuments 286 两核心编排例外）。验证：lint 0 + tsc 0 + build **350 modules**（+13 文件，CSS 65.60 kB 不变）+ file-size OK（2 基线）+ 负向探针 + 浏览器 smoke（smoke-vault-local-mount OK：登录 alice → documents → `.local-mount-pane` 渲染）；CI PR #162 **8 job 全绿**；PR #162 squash merge main `9f5c967`，bump v3.8.22。实施口径 `tasks/task-057-frontend-component-split-slice-e.md`
- **维护态批26（Sprint-51，2026-08-13·CQ-P1-006 后续）**：前端 codegen Slice A 试点——`openapi-typescript@^7.13.0`（devDep）+ `gen:api` 脚本（`openapi-typescript ../openapi/openapi.json -o src/api/generated.ts`，避开 `--immutable` v7 bug #1368）生成 `frontend/src/api/generated.ts`（入库）；`tags.ts` 混合接入（TagView `Omit<生成,'status'> & {status: TagStatus}` narrow / DocumentTagView·TagLinkView 零差异 alias / ListEnvelope 手写保留）；CI `frontend-schema-diff` job（advisory，复用 schema-diff 的 `git diff --exit-code` 模式）；`check-frontend-file-size.mjs` 豁免 codegen 产物（`*generated.ts` 机器生成）。**接入策略=混合接入**（Explore 评估揭示 openapi 几乎无 enum，全量替换丢 ~12 处 union narrow；documents Summary|Detail 合并留 Slice B 单独批）。验证：gen 幂等 drift 0 + lint 0 + build **350 modules** 0 error（零回归）+ file-size OK + tags 消费方 tsc 零回归；浏览器 smoke 留 Slice B。CI PR #165 **9 job 全绿**（含新 frontend-schema-diff 首跑）；PR #165 squash merge main `1a06f8d`，bump v3.8.23。实施口径 `tasks/task-058-frontend-codegen.md`
- **维护态批27（Sprint-52，2026-08-13·CQ-P1-006 后续）**：前端 codegen Slice B-1 内容域混合接入——`folders`/`docLinks`/`timeline`/`search` 4 模块（tags 已 Slice A）：主体 alias 生成类型 + union（status/event_type/level/permission/source_type）保留手写 narrow + 分页/请求体/前端专属手写；folders `FolderDetail` 清理冗余 created_at/updated_at（后端 schema 不返回、grep 确认无消费方）；timeline 嵌套 narrow（items/density/window）；search 命名错位 alias（Result↔ResultView / Response↔PageView / Meta↔View）+ RagSource optionality。接入后类型定义行数减（alias 替代手写，4 文件 +45/-74）。验证：lint 0 + build **350 modules** 0 error（零回归）；CI PR #166 **9 job 全绿**；PR #166 squash merge main `59dea01`，bump v3.8.24。实施口径 `tasks/task-058-frontend-codegen.md`
- **维护态批28（Sprint-53，2026-08-13·CQ-P1-006 后续）**：前端 codegen Slice B-2 术语导入域混合接入——6 域（**范围修正**：原计划 7 域中 rag 已随 B-1 search.ts 完成）：`terms`（TermStatus alias 生成 enum + `Term=Omit<TermDetail,'status'>` narrow + TermWritePayload=TermWriteRequest 零差异 alias + TermListResponse 手写保留）/ `termCategories`（View/Detail/ListPage 零差异 alias）/ `quickEntry`（QuickEntryView status narrow）/ `aiPolish`（PolishView status narrow + PolishSource alias）/ `imports`（ImportResponse=ImportFileView 命名错位 + ImportBatchItem status narrow + ImportBatchResponse 嵌套 narrow + failedBatchFromSlice 补必填 null + File/FormData payload 手写）/ `exports`（PdfExportResponse status narrow）；union / 分页容器 / 前端专属请求体按混合接入策略保留手写，6 文件 +52/-90。验证：lint 0 + build **350 modules** 0 error + 浏览器 smoke PASS（术语树/详情 + 快速录入 converted 分支 + AI 润色面板，无运行时错误）；CI PR #167 **9 job 全绿**；PR #167 squash merge main `397382f`，bump v3.8.25。实施口径 `tasks/task-058-frontend-codegen.md`
- **维护态批29（Sprint-54，2026-08-13·CQ-P1-006 后续）**：前端 codegen Slice B-3 账户空间域混合接入——4 域（**范围修正**：config 域已随 B-1 search.ts、users 域本在 spaceMembers.ts）：`auth`（LoginResponse=Omit<LoginView,'current_space_id'|'role'> narrow——后端 `number|null`/裸 string → 前端 number/union，运行时哨兵=注册即建个人空间；RegisterResponse email narrow；SessionInfo/PasswordResetMessageView 零差异 alias）/ `spaces`（Space=SpaceView / SwitchSpaceView 零差异 alias）/ `spaceMembers`（SpaceMemberView role narrow + UserSearchResult=UserSearchView 命名错位 alias）/ `admin`（AdminUserView role/status narrow + AdminUserSpaceView role narrow 命名错位 + AdminUserSpaceAvailable alias + AdminUserSpacesResult 嵌套 narrow）；union / 请求体按混合接入策略保留手写，4 文件 +50/-56。验证：lint 0 + build **350 modules** 0 error（bundle 逐字节不变，纯类型层）+ 浏览器 smoke PASS（登录/空间切换/成员表/用户管理/用户空间抽屉，无运行时错误）；CI PR #168 **9 job 全绿**；PR #168 squash merge main `90d06d5`，bump v3.8.26。实施口径 `tasks/task-058-frontend-codegen.md`
- **维护态批30（Sprint-55，2026-08-13·CQ-P1-006 后续·Slice B 全量收口）**：前端 codegen Slice B-4 documents——`KnowledgeDocument = DocumentSummaryView|DocumentDetailView` 真 union（替代旧 `content_md?: string` 压平哨兵）+ `isDocumentDetail` 守卫 + `DocumentPermission` alias 生成 enum + 函数签名精确化（list→Summary[] / 详情与写操作→Detail）；`useDocuments`/`useDocumentSideData` 5 处哨兵改 narrow；**契约失配修正**：`listDocumentsByTag` 后端实际瘦身 `DocumentTagItemView[]`（旧手写谎称完整文档）→ `TaggedDocumentItem` narrow；CI `frontend-schema-diff` 升 **required**（前端 codegen Slice A+B 17 域全量闭环）。7 文件 +73/-54。验证：lint 0 + build **350 modules** 0 error + 浏览器 smoke 全链路 PASS（list→详情→草稿回填→编辑保存→移动→版本→标签打开，fixture 自建自清）；CI PR #169 **9 job 全绿**（project-check ratchet 曾拦 useDocuments 286→287，import 合并修正复绿）；PR #169 squash merge main `f66b00f`，bump v3.8.27。发现的「AI 生成代码类型撒谎」模式记 pitfall `.ai/pitfalls/2026-08-13-ai-code-type-lies.md`（候选模板提案）。实施口径 `tasks/task-058-frontend-codegen.md`
- 桌面端浏览器访问
