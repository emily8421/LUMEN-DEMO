...
- **维护态批8（Sprint-33，2026-08-12·Slice A 已完成 PR #139）**：P1 repository 契约（CQ-P1-002）Slice A——`RepositoryProtocol` 契约化（`backend/repository/protocol.py` typing.Protocol + @runtime_checkable + 98 方法 inspect 机器提取自 PgRepository + pg/demo 显式继承 + contract test 守护双实现一致）；Slice B（按域拆 god object）/ Slice C（service repository 参数注解）待续；实施口径见 `docs/research/2026-08-10-code-governance-rollout-plan.md` §4 轨道3 + §5 轨道C
- 桌面端浏览器访问
