...
- **维护态批7（Sprint-32，2026-08-11·Slice A 已完成 v3.8.1 PR #125）**：P1 错误契约收口（CQ-P1-005，NFR-007）Slice A——错误响应契约地基（业务码单一含义 + 兜底 5xx envelope + `ApiError` 基类 + 集中 `code→HTTP` 映射 + 禁 `str(exc)`）；`backend/model/error_codes.py` + `main.py` 双 handler + TestClient 断言；Slice B（api `str(exc)` 清零 + ~40 异常迁移继承 `ApiError` + 散落 `_status_for` 收口）/ Slice C（前端 `client.ts` `ApiError` + `session-store` 删文案判 auth）待续；实施口径见 `docs/research/2026-08-10-code-governance-rollout-plan.md` §4 轨道3
- 桌面端浏览器访问
